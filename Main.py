...
def EML():
    ...
    prediction_data = prediction(X_test, cls)
    classifier = cls
    eml_acc = cal_accuracy(y_test, prediction_data,'Extreme Machine Learning Algorithm Accuracy, Classification Report & Confusion Matrix')
...
def predict():
    ...
    for i in range(len(predict)):
        if predict[i] == 0:
            text.insert(END,"Test Data : "+str(test[i])+" =====> No Heart Disease PREDICTED\n\n")
        if predict[i] == 1:
            text.insert(END,"Test Data : "+str(test[i])+" =====> Heart Disease PREDICTED\n\n")    
